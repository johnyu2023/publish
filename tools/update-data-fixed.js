...
const CATEGORY_FILE = path.join(__dirname, '../docs-source/data/category.json');
...
function traverseDirectory(dir, baseDir) {
  ...
  if (relativePath && !IGNORE_DIRS.includes(dirName)) {
    // 避免重复添加目录
    if (!result.directories.includes(dirName)) {
...
function main() {
  ...
  // 写入结果到文件，使用 UTF-8 编码
  fs.writeFileSync(OUTPUT_FILE, JSON.stringify(result, null, 2), { encoding: 'utf8' });
  
  // 写入分类信息到文件，使用 UTF-8 编码
  fs.writeFileSync(CATEGORY_FILE, JSON.stringify(categories, null, 2), { encoding: 'utf8' });
  
  console.log(`已更新数据文件: ${OUTPUT_FILE}`);
  console.log(`- 目录数量: ${result.directories.length}`);
  console.log(`- 文章数量: ${result.articles.length}`);
  
  console.log(`已更新分类文件: ${CATEGORY_FILE}`);
}
